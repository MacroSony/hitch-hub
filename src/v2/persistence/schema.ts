/**
 * The one and only first-slice SQLite layout.
 *
...
const HITCH_V2_SCHEMA_DDL_UNQUALIFIED = [
  ...
  `CREATE TABLE principals (
    ...
    disabled_actor_system_component TEXT CHECK (disabled_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
    created_at TEXT NOT NULL,
  ...
  `CREATE TABLE identity_bindings (
    ...
    source_kind TEXT NOT NULL CHECK (source_kind = 'local-peer'),
    ...
    revoked_actor_system_component TEXT CHECK (revoked_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
    supersedes_binding_id TEXT REFERENCES identity_bindings(id),
    created_at TEXT NOT NULL,
    UNIQUE (id, principal_id),
    UNIQUE (id, installation_id),
    CHECK (local_host_id IS NOT NULL),
  ...
  `CREATE TABLE authentication_requests (
    id TEXT PRIMARY KEY,
    installation_id TEXT NOT NULL REFERENCES installations(id),
    evidence_kind TEXT NOT NULL CHECK (evidence_kind = 'local-peer-owner-socket'),
    socket_security TEXT NOT NULL CHECK (socket_security = 'service-owned-0700-parent-and-0600-socket'),
    ...
    CHECK ((outcome_status = 'authenticated' AND principal_id IS NOT NULL AND identity_binding_id IS NOT NULL AND assurance IS NOT NULL AND rejection_reason IS NULL) OR (outcome_status = 'rejected' AND principal_id IS NULL AND identity_binding_id IS NULL AND assurance IS NULL AND rejection_reason IS NOT NULL))
  )`,
  `CREATE TABLE endpoints (
    id TEXT PRIMARY KEY,
    installation_id TEXT NOT NULL REFERENCES installations(id),
    address_kind TEXT NOT NULL CHECK (address_kind = 'local-client'),
    local_host_id TEXT NOT NULL REFERENCES local_hosts(id),
  ...
  `CREATE TABLE access_grants (
    ...
    granted_actor_system_component TEXT CHECK (granted_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
    created_at TEXT NOT NULL,
    state TEXT NOT NULL CHECK (state IN ('active', 'revoked')),
    revoked_at TEXT,
    revoked_actor_kind TEXT,
    revoked_actor_principal_id TEXT REFERENCES principals(id),
    revoked_actor_system_component TEXT CHECK (revoked_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
    CHECK ((kind = 'installation-role' AND installation_id IS NOT NULL AND role = 'admin' AND resource_kind IS NULL AND resource_id IS NULL) OR (kind = 'session-configuration-use' AND installation_id IS NOT NULL AND role IS NULL AND resource_kind IS NOT NULL AND resource_id IS NOT NULL)),
  ...
  `CREATE TABLE provider_credential_bindings (
    ...
    created_actor_system_component TEXT CHECK (created_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    UNIQUE (id, provider_id),
  ...
  `CREATE TABLE session_endpoint_bindings (
    ...
    suspended_actor_system_component TEXT CHECK (suspended_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
  ...
  `CREATE TABLE turn_runtime_states (
    ...
    requested_actor_system_component TEXT CHECK (requested_actor_system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
  ...
  `CREATE TABLE audit_envelopes (
    ...
    system_component TEXT CHECK (system_component IN ('application', 'authorization', 'bootstrap', 'broker', 'delivery', 'local-connector', 'recovery', 'sidecar', 'supervisor', 'turn-coordinator')),
  ...
  `CREATE UNIQUE INDEX uq_identity_bindings_active_subject ON identity_bindings (installation_id, subject_id) WHERE state = 'active'`,
  ...
  `CREATE UNIQUE INDEX uq_access_grants_active_installation_role ON access_grants (installation_id, principal_id, role) WHERE kind = 'installation-role' AND state = 'active'`,
  ...
  `CREATE INDEX ix_turn_queue_entries_head ON turn_queue_entries (session_id, queue_position)`,
